            ...
            if date_time_obj < previous_minute.strftime("%Y-%m-%d %H:%M"):
                ...
                if domain == "en.wikipedia.org":
                    if "performer" in response and "user_edit_count" in response:
                        wiki_user = list(
                            filter(lambda report_data: report_data['user'] == response["performer"]["user_text"],
                                   user_report))
                        if wiki_user:
...
def bonus_task():
    current_time = datetime.datetime.now()
    five_minute = datetime.timedelta(hours=5, minutes=25, seconds=5)
    previous_minute = current_time - five_minute
    # this will ensure that previous 5 minute data has to be considered
    print("Last Five Minutes report : ")
    generate_reports(previous_minute)
    pass


# # # report would be generated every minute
schedule.every(0).minutes.do(task)
schedule.every(0).minutes.do(bonus_task)
# #
while True:
